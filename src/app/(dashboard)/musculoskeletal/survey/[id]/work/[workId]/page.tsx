...
function ElementWorkDetailPage() {
  ...
  // State for force/static/repetition per body part
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Initialize forceStatic data from existing scores
        const initialForceStatic: Record<string, { forceChecked: boolean; staticOver1min: boolean; repetitionChecked: boolean }> = {}
        BODY_PARTS.forEach((part) => {
          const existingScore = workData.elementWork.bodyPartScores?.find(
            (s: BodyPartScore) => s.bodyPart === part.id
          )
          const fs = (existingScore?.additionalFactors as Record<string, unknown>)?._forceStatic as Record<string, boolean> | undefined
          initialForceStatic[part.id] = {
            forceChecked: fs?.forceChecked ?? false,
            staticOver1min: fs?.staticOver1min ?? false,
            repetitionChecked: fs?.repetitionChecked ?? false,
          }
        })
        ...
        <>
          ...
          <div className="space-y-3">
            {BODY_PARTS.map((part) => {
              ...
              const displayScore = realtimeScore || (existingScore ? {
                postureScore: existingScore.postureScore,
                additionalScore: existingScore.additionalScore,
                forceScore: 0,
                staticRepetitionScore: 0,
                totalScore: existingScore.totalScore,
              } : null)
              const scoreLevel = displayScore ? getScoreLevel(displayScore.totalScore) : null
                        ...
                        {displayScore && (
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`text-xs px-2 py-0.5 rounded ${levelInfo?.color || ''}`}>
                              {displayScore.totalScore}점 ({levelInfo?.label})
                            </span>
                            <span className="text-xs text-gray-500">
                              자세{displayScore.postureScore} + 부가{displayScore.additionalScore} + 힘{displayScore.forceScore} + 정적/반복{displayScore.staticRepetitionScore}
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    <CardContent className="border-t bg-gray-50 space-y-6">
                      ...
                      {realtimeScore && (
                        <div className="bg-white rounded-lg border p-4">
                          <h4 className="text-sm font-medium text-gray-700 mb-2">실시간 점수 계산</h4>
                          <div className="flex items-center gap-4 text-sm">
                            <span className="text-gray-600">자세: <strong>{realtimeScore.postureScore}</strong></span>
                            <span className="text-gray-400">+</span>
                            <span className="text-gray-600">부가: <strong>{realtimeScore.additionalScore}</strong></span>
                            <span className="text-gray-400">+</span>
                            <span className="text-gray-600">힘: <strong>{realtimeScore.forceScore}</strong></span>
                            <span className="text-gray-400">+</span>
                            <span className="text-gray-600">정적/반복: <strong>{realtimeScore.staticRepetitionScore}</strong></span>
                            <span className="text-gray-400">=</span>
                            <span className={`font-bold text-lg px-2 py-0.5 rounded ${levelInfo?.color || ''}`}>
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-3">힘 / 정적자세 / 반복성</h4>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {BODY_PARTS.map((part) => {
                  const realtimeScore = calculateRealtimeScore(part.id)
                  const savedScore = elementWork.bodyPartScores.find((s) => s.bodyPart === part.id)
