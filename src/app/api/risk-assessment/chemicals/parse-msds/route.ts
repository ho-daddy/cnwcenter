...
interface MsdsExtractedData {
  ...
  productHazards: string       // Section 2 유해성·위험성 (제품 단위)
  productRegulations: string   // Section 15 등 제품 규제사항
}
...
4. 제품 단위 유해성·위험성 (Section 2 "유해성·위험성" / "Hazards Identification"):
   - GHS 분류 항목과 구분을 한 줄에 하나씩 원문 그대로 나열
   - 예: "발암성: 구분1B", "급성 독성(경구): 구분4", "심한 눈 손상성/눈 자극성: 구분2"
   - 영문 문서면 한국 GHS 표기로 번역 (예: "Carcinogenicity Category 1B" → "발암성: 구분1B")
   - 신호어, 그림문자 코드(GHS01 등), H문구는 제외하고 분류 항목만
5. 구성성분 목록 (Section 3 "구성성분의 명칭 및 함유량"):
   - CAS 번호 (예: 67-64-1)
   - 화학물질명 (한글 또는 영문)
   - 함유량/농도 (백분율. 범위값 포함. 예: "10~30", "85", "비공개", "영업비밀")
6. 취급 및 저장방법 (Section 7 "취급 및 저장방법" / "Handling and Storage")
7. 노출방지 및 개인보호구 (Section 8 "노출방지 및 개인보호구" / "Exposure Controls/Personal Protection")
8. 제품 규제사항 (Section 15 "법적 규제현황" / "Regulatory Information"):
   - 산업안전보건법상 분류(관리대상유해물질, 특별관리물질 등) 또는 화학물질관리법상 분류
   - 해당 사항이 없으면 빈 문자열
...
- handlingStorage, exposureProtection, productHazards, productRegulations는 해당 섹션의 내용을 가능한 그대로 추출하세요.
- productHazards가 없으면 빈 문자열을 반환하세요.
...
응답 JSON 형식:
{
  ...
  "productRegulations": "섹션 15 법적 규제현황 원문",
  "warnings": ["불확실한 사항 설명"]
}`
...
async function parseMsdsResponse(response: { content: Array<{ type: string; text?: string }> }): Promise<MsdsExtractedData> {
  ...
  return {
    ...
    warnings: Array.isArray(parsed.warnings)
      ...
      : [],
    managementMethod,
    productHazards: String(parsed.productHazards || '').trim(),
    productRegulations: String(parsed.productRegulations || '').trim(),
  }
}
