...
export default function PublicSurveyPage() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        if (!res.ok) {
          setError(res.status === 404 ? '설문을 찾을 수 없거나 마감되었습니다.' : '설문을 불러오는 중 오류가 발생했습니다.')
          return
        }
  ...
  const handleSubmit = async () => {
    if (!validateCurrentStep() || !survey) return
    setSubmitting(true)
    try {
      // questionCode 기반 answers → questionId 기반으로 변환
      const answersByQuestionId: Record<string, unknown> = {}
      for (const [key, val] of Object.entries(answers)) {
        const qId = codeToId[key] || key
        answersByQuestionId[qId] = val
      }
        ...
        {visibleQuestions.map(q => {
          ...
          return (
            <QuestionField
              ...
              value={answers[code]}
              error={errors[code]}
              onChange={(val) => setAnswer(code, val)}
            />
          )
        })}
      </div>
        ...
        <div className="max-w-2xl mx-auto flex justify-between">
          ...
          {isLastStep ? (
            <button onClick={handleSubmit} disabled={submitting}
              className="flex items-center gap-1 px-6 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
...
function CheckboxField({ options, value, onChange }: { options: QuestionOption[]; value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      {options.map(opt => (
        <label key={opt.value} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-gray-50">
          <input type="checkbox" checked={current.includes(opt.value)}
