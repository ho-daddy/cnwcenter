 ...
 * 안전보건뉴스 섹션
 */
export class Safety1stNewsScraper extends BaseScraper {
  private baseUrl = 'https://www.safety1stnews.com'
    ...
    try {
      ...
      $('h4.titles a').each((_, el) => {
        try {
          ...
          console.error('[Safety1stNewsScraper] 항목 파싱 실패:', err)
        }
      })
    } catch (err) {
      console.error('[Safety1stNewsScraper] 수집 실패:', err)
      throw err
    }
